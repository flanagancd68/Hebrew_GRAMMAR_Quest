...
## How do I know when to hit the `Good` button on a **Vocabularly** or **Grammar** card? {-}
...
* You do not necessarily need to be able to translate the entire passage from memory; if you can comfortably read the Hebrew and your translation is roughly similar to the English answer (it doesn't have to be exact) you should hit `good`
1. If your translation is missing one or more words when compared to the English answer (again excluding roughly equivalent meanings), you should select `again`
1. If you need to use a `hint`, you should select `again` until you can translate without needing to refer to the hint.
1. We do not ever recommend selecting `easy` unless you are confident that you know the passage so well from memory that you do not need to see it again for a long time.

Most importantly, be patient with yourself.  It may take you many tries in the early going before you are able to hit `good` - learning a new language is not easy and you are to be commended for the Hebrew learning road you are now taking!
...
Short answer is `yes`, but you would need to learn more about how Anki works.  Such a discussion is beyond our scope with this book but if you are interested in learning more about Anki there are dozens of YouTube videos.
...
1. If you have added images and want to be tested on Hebrew-to-English, English-to-Hebrew, and Image-to-Hebrew, select note type `Vocab Basic Hebrew/English/Image`
...
Anki is doing exactly what we told it to do in the `settings`!  As much as possible, we want to let Anki tell _us_ when to work.

* We go when it tells us there are cards to learn or review
* We stop, when it tells us there are no cards

1. We set new cards to `200` - it's important not to overload yourself.
    ...
    * You can keep going using the `Custom Study` - but note the warning: `You can increase the limit in the options, but please bear in mind that the more new cards you introduce, the higher your short-term review workload will become`
2. We checked `bury related cards`
    * If an answer has two parts in two separate questions, we want to make sure you answer the second question from your knowledge, not because you just saw the answer on a related card
